...
function accessPoints(place, slow){
  if(COORD[place]) return [{stop:place, mins: slow ? 3 : 2, self:true}];
...
function crowdScore(lineKey, fromSt, toSt, minutes){
  ...
  return Math.max(0,Math.min(1, base*dir*(LINES[lineKey]?.load??0.8)));
}
...
for(const [k,c] of Object.entries(BUS_CORRIDORS))
  for(let i=0;i<c.stops.length-1;i++){
    ...
    link(a,b,c.hop,{kind:"ride",mode:"bus",line:k});
    link(b,a,c.hop,{kind:"ride",mode:"bus",line:k});
  }
...
function route(fromSt,toSt,departMin,prof,opts){
  ...
  const platformsAt = place => {
    ...
    if(modes.bus)  for(const k of (BUS_STOPS.get(place)||[])) out.push(nid("BUS:"+k,place));
    return out;
  };
  const destStops = new Map(destAccess.map(a=>[a.stop,a.mins]));

  const dist=new Map(),prev=new Map(),realT=new Map(),pq=[];
  const push=(n,c)=>{ pq.push([c,n]); pq.sort((a,b)=>a[0]-b[0]); };
  ...
  while(pq.length){
    ...
    for(const e of (GRAPH.get(node)||[])){
      ...
      if(e.kind==="ride"){
        ...
        pen += mins*(prof.transfer-1) + prof.transfer*2.2;
  ...
  for(const l of legs){
    ...
    if(l.mode==="rail"){
      l.crowd=crowdScore(l.line,l.from,l.to,l.start);
      ...
      l.issues=FEED.filter(d=>busHit(d,l.line));
    }
  }
  /* closing walk */
